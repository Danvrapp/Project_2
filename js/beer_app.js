...
function filterStyle(event) {
    ...
    tableData.forEach(function(beer) {
        if (beer.style == theStyle) { //iterate through data looking to match user's date
            flag = 1;  //found data!
...
function filterRating(event) {
    ...
    tableHeaders();  //add table headers back in
    tableData.forEach(function(beer) {
        if (beer.rating >= theRating) { //iterate through data looking to match user's rating
            flag = 1; //found data!
            var row = tbody.append("tr"); //add a row to the (new) table
            Object.entries(beer).forEach(function([key, value]) {
                var cell = row.append("td"); //add a column to the row
...
function filterState() {
    ...
    tableHeaders();  //add table headers back in
    tableData.forEach(function(beer) {
        if (beer.state == theState) {  //iterate through data looking to match user's state
            flag = 1;  // found data!
            var row = tbody.append("tr");  //add a row to the (new) table
            Object.entries(beer).forEach(function([key, value]) {
                var cell = row.append("td");  //add a column to the row
